...
""")
    ...
    def _safe_close(self):
        """Close TraCI safely across different SUMO versions."""
        try:
            if traci.isLoaded():
                traci.close()
                time.sleep(0.2)
        except Exception:
            # Some SUMO versions raise AttributeError on _socket during close
            # Kill any lingering connection by closing all known connections
            try:
                for label in list(traci._connections.keys()):
                    try:
                        traci._connections[label]._socket = None
                    except Exception:
                        pass
                traci._connections.clear()
                traci._currentLabel[0] = ""
            except Exception:
                pass
            time.sleep(0.2)
